...
public:
	...
	void calculateFlatNormals() {
		...
		for(int i = 0; i < indices.size(); i += 3) {
			...
			ofVec3f a = points[indices[i]];
			ofVec3f b = points[indices[i+1]];
			ofVec3f c = points[indices[i+2]];
			
			// calculate the normal for the triangle
			ofVec3f normal = (a-b).getCrossed(c-b);
			normal.normalize();
			normal *= -1;
	...
	void calculateSmoothNormals() {
		...
		switch(mode) {
			...
			case 0 :
				...
				for ( i=0; i<Vertices; i++ ) normals[i] = ofVec3f(0, 0, 0);
...
//					Surface = Mesh->SurfaceList [ i ];
	...
	void drawFlat() {
		...
		glVertexPointer( 3, GL_FLOAT, sizeof(ofVec3f), &flatPoints[0].x );
		glNormalPointer(GL_FLOAT, sizeof(ofVec3f), &flatNormals[0].x );
	...
	vector<ofVec3f> flatPoints;
	vector<ofVec3f> flatNormals;
